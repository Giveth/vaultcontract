...
var Escapable = artifacts.require('../contracts/Escapable.sol');
var TestStandardToken = artifacts.require('../contracts/helpers/TestStandardToken.sol');
...
contract('Escapable', accounts => {
  ...
  it('changeEscapeHatchCaller() changes the permission', async () => {
    let result = await escapableEth.changeEscapeHatchCaller(OTHERADDR,{from: ESCAPEHATCHCALLERADDR})
    ...
    result = await escapableEth.changeEscapeHatchCaller(ESCAPEHATCHCALLERADDR,{from: OTHERADDR});
  ...
  it('ether: receiveEther() generates a log', async () => {
    let result = await escapableEth.receiveEther({from: OTHERADDR, value : ONEWEI})
    let logs = filterCoverageTopics(result.logs)
    assert.equal( logs.length, 1);
  ...
  it('ether: escapeHatch() sends amount to the destination', async () => {
    ...
    let result = await escapableEth.escapeHatch({from: ESCAPEHATCHCALLERADDR});
    let logs = filterCoverageTopics(result.logs)
    assert.equal( logs.length, 1);
  ...
  it('token: escapeHatch() sends amount to the destination', async () => {
    ...
    let result = await escapableToken.escapeHatch({from: ESCAPEHATCHCALLERADDR});
    let logs = filterCoverageTopics(result.logs)
    assert.equal( logs.length, 1);
    assert.equal( logs[0].event, "EscapeHatchCalled");
